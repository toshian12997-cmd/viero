...
import { ArrowRight, Bell, BriefcaseBusiness, Check, ChevronRight, CircleHelp, Database, FileText, LayoutDashboard, LogOut, Menu, Package, Search, Settings, ShoppingCart, Sparkles, Users, WalletCards, X } from 'lucide-react';

const navItems = [
  ['Overview', LayoutDashboard], ['Sales', ShoppingCart], ['Customers', Users], ['Inventory', Package], ['Invoices', FileText], ['Reports', WalletCards], ['Decision Room', BriefcaseBusiness], ['VIERO AI', Sparkles]
];
...
function Modal({ modal, close }) {
  ...
  return <div className="modal" onMouseDown={e => e.target === e.currentTarget && close()}><div className="modal-card"><button className="modal-close" onClick={close}><X size={18}/></button><span className="eyebrow">{modal.label}</span><h2>{modal.title}</h2><div className="modal-body">{modal.body}</div><button className="modal-action" onClick={close}>Close</button></div></div>;
}
...
function Intro({ onDone }) {
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => setStage(2), 7800),
      setTimeout(() => setStage(3), 9400),
      setTimeout(() => setStage(4), 10800),
      setTimeout(onDone, 17000)
    ];
    return () => timers.forEach(clearTimeout);
  }, [onDone]);
  return <section className={`intro stage-${stage}`}>
    <div className="intro-scene"><div className="scene-light"/><div className="floor"/>
      <div className="gate"><span/><i/></div>
      <div className="walker"><div className="head"/><div className="neck"/><div className="torso"><em>V</em></div><div className="arm left"/><div className="arm right"/><div className="leg left"/><div className="leg right"/><div className="case"><i/></div></div>
      <div className="case-glow"/>
    </div>
    <div className="intro-brand"><Logo/><span>BUSINESS INTELLIGENCE • DECISION • ACTION</span></div>
...
function Auth({ onLogin }) {
  ...
  return <main className="auth">
    <div className="auth-image"><div className="image-shade"/><div className="image-caption"><span>VIERO</span><h2>Understand the business behind the numbers.</h2><p>Built for the decisions that move a business forward.</p></div></div>
    <section className="auth-panel"><Logo/><span className="eyebrow">VALUE, ENTERPRISE, YIELD &amp; REVENUE OPTIMIZATION</span><h1>Know your business.<br/><strong>Decide with confidence.</strong></h1><p className="auth-copy">A business command centre for understanding performance, finding constraints and making better decisions.</p>
      ...
      <div className="divider">New to VIERO?</div><button className="secondary" type="button">Create business account</button><p className="legal">By continuing, you agree to VIERO's Terms and Privacy Policy.</p>
    </section>
  </main>;
}
...
export default function App() {
  ...
  useEffect(() => { if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) setView('auth'); }, []);
  return <>{view === 'intro' && <Intro onDone={() => setView('auth')}/>} {view === 'auth' && <Auth onLogin={() => setView('app')}/>} {view === 'app' && <Dashboard logout={() => setView('auth')}/>}</>;
}
